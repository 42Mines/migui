...
# X: tensor of shape [100+, 64, 64, 1] containing a batch of images (float32)
...
# return value: tensor of shape [640, 640, 3] containing the 100 first unrecognised images (rgb, uint8)
...
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    ...
    for imslice in range(lines):
        Islices.append(tf.concat(tf.unstack(tf.slice(I, [imslice * n // lines, 0, 0, 0], [n // lines, 64, 64, 3])), 1))
...
class MnistDataVis:
    xmax = 0
    y2max = 0
    # a list containing accuracy on positive labels, for each test session
    acc_pos_ls = []
    # a list containing accuracy on negative labels, for each test session
    acc_neg_ls = []
    x1 = []
    y1 = []
    z1 = []
    ...
    im1 = np.full((64 * 10, 64 * 10, 3), 255, dtype='uint8')
    im2 = np.full((64 * 10, 64 * 10, 3), 255, dtype='uint8')
    _animpause = False
    _animation = None
    _mpl_figure = None
    _mlp_init_func = None
    ...
    def __init__(self, title1=None, title2=None, title3=None, title4=None, title5=None, title6=None,
        ...
        self.__set_title(ax5, title5, default="Précision sur les positifs et négatifs")
        ...
        line5, = ax5.plot(self.x1, self.acc_pos_ls, label="précision sur les positifs")
        line6, = ax5.plot(self.x2, self.acc_neg_ls, label="précision sur les négatifs")
        legend = ax5.legend(loc='lower right')  # fancybox : slightly rounded corners
        legend.draggable(True)
        ...
        def _init():
            ...
            ax5.set_ylim(0, 1)  # important: not autoscaled
        ...
        def _update():
            ...
            # 6 curves: train and test accuracy, train and test loss, pos and neg accuracy
            line1.set_data(self.x1, self.y1)
            line2.set_data(self.x2, self.y2)
            line3.set_data(self.x1, self.z1)
            line4.set_data(self.x2, self.z2)
            line5.set_data(self.x1, self.acc_pos_ls)
            line6.set_data(self.x2, self.acc_neg_ls)
    ...
    def append_test_curves_data(self, x, accuracy, accuracy_pos, accuracy_neg, loss):
        self.x2.append(x)
        self.y2.append(accuracy)
        self.acc_pos_ls.append(accuracy_pos)
        self.acc_neg_ls.append(accuracy_neg)
        self.z2.append(loss)
        self._update_xmax(x)
    ...
    def get_average_test_accuracy(self):
        """ :returns the average accuracy on the ten last tests"""
        if len(self.y2) >= 10:
            data = self.y2[::-1]  # inverse l'ordre
            data = data[:10]  # récupère les 10 derniers éléments de test
            avg = sum(data)/10
            return avg
        else:
            print("pas assez de données pour obtenir la précision moyenne")

    def get_average_test_accuracy_pos(self):
        """ :returns the average accuracy on positive images on the ten last tests"""
        if len(self.acc_pos_ls) >= 5:
            data = self.acc_pos_ls[::-1]  # inverse l'ordre
            data = data[:5]  # récupère les 10 derniers éléments de test
            avg = sum(data)/5
            return avg
        else:
            print("pas assez de données pour obtenir la précision moyenne sur les positifs")
            return None

    def get_average_test_accuracy_neg(self):
        """ :returns the average accuracy on negative images on the ten last tests"""
        if len(self.acc_neg_ls) >= 5:
            data = self.acc_neg_ls[::-1]  # inverse l'ordre
            data = data[:5]  # récupère les 10 derniers éléments de test
            avg = sum(data)/5
            return avg
        else:
            print("pas assez de données pour obtenir la précision moyenne sur les négatifs")
            return None
